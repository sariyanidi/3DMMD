...
class PerspectiveFitter():
    ...
    def __init__(self, mm, cam, F=7, use_ineq=False, use_maha=True, which_pts='all', 
        ...
        if self.which_pts == 'lmks':
            ...
            NALLPTS = len(self.mm.mean_shape[::3])
            sampled = torch.range(0, NALLPTS-1, 30).long().to(self.mm.device)
            self.pts_indices = torch.cat([self.mm.li, sampled])
        
        x0 = self.mm.mean_shape[::3][self.pts_indices].reshape(-1,1).cpu().numpy()
        y0 = self.mm.mean_shape[1::3][self.pts_indices].reshape(-1,1).cpu().numpy()
        z0 = self.mm.mean_shape[2::3][self.pts_indices].reshape(-1,1).cpu().numpy()
        ...
        if self.which_basis == 'identity':
            self.alpha_ub = 9*self.mm.sigma_alphas
            self.alpha_lb = -9*self.mm.sigma_alphas
        elif self.which_basis == 'expression':
            self.alpha_ub = self.mm.eps_upper
            self.alpha_lb = self.mm.eps_lower
    ...
    def compute_gradient_and_hessian_f(self, x, lmks, f):
        ...
        R, dR_du = utils.get_rot_matrix(u.cpu().numpy())
        R = torch.from_numpy(R).to(self.mm.device).float()
        dR_du = torch.from_numpy(dR_du).to(self.mm.device).float()
        
        if self.which_basis == 'identity':
            alpha = alpha.unsqueeze(0).to(self.mm.device)
            exp = None
        elif self.which_basis == 'expression':
            exp = alpha.unsqueeze(0).to(self.mm.device)
        ...
        nablaWx = torch.zeros(self.NPTS,len(x))
        ...
        nablaWy = torch.zeros(self.NPTS,len(x))
        ...
        diffx = (xproj.cpu().numpy() - lmks[self.pts_indices.cpu(),0:1])
        diffy = (yproj.cpu().numpy() - lmks[self.pts_indices.cpu(),1:2])
        
        # print(diffx.shape)
        # print(diffy.shape)

        nablaWx = nablaWx.cpu().numpy()
        nablaWy = nablaWy.cpu().numpy()
        ...
        if self.use_ineq and f == 0:
            ...
            f_alpha_ub = alpha.T-self.alpha_ub
            f_alpha_lb = self.alpha_lb-alpha.T
            f_alpha_ub = f_alpha_ub.cpu().numpy()
            f_alpha_lb = f_alpha_lb.cpu().numpy()
            
            if self.use_maha:
                cmaha = (alpha*((self.mm.sigma_alphas.cpu().numpy())**(-2))*alpha).sum().cpu().numpy()
                f_maha = cmaha-self.maha2_threshold
                # print(f'{f_maha:.3f} == ')
                nabla_f_maha = -(1.0/f_maha)*2*alpha*((self.mm.sigma_alphas.cpu().numpy())**(-2))
                nabla_f_maha = nabla_f_maha.reshape(-1,1).cpu().numpy()
            
            nablaf_alpha_ub =  np.diag(-1.0/f_alpha_ub)
            nablaf_alpha_lb = -np.diag(-1.0/f_alpha_lb)
            
            nablaf_alpha = np.sum(nablaf_alpha_ub+nablaf_alpha_lb,axis=0)
            ...
            obj += - np.sum(np.log(-f_alpha_lb)) - np.sum(np.log(-f_alpha_ub))
            if self.use_maha:
                obj += - np.log(-f_maha)
    ...
    def evaluate_function_f(self, x, lmks, f):
        ...
        R, _ = utils.get_rot_matrix(u.cpu().numpy())
        R = torch.from_numpy(R).to(self.mm.device).float()
    ...
    def fit_GN(self, x0, all_lmks, use_ineq=False, plotit=False, plotlast=False):
        ...
        for i in range(MAXITER):
            ...
            H[:self.num_components,:self.num_components] += dlt*np.eye(self.num_components)
            ...
            if plotit:
                ...
                plt.plot(all_lmks[0][:,0], all_lmks[0][:,1], 'o')
            ...
            terminate = False
            while it < maxiter:
                xtmp = x.cpu().numpy() + t * search_dir.reshape(-1,)
                obj_tmp, _ = self.evaluate_function(torch.from_numpy(xtmp).float(), all_lmks)
                # print(obj_tmp)
                
                if obj_tmp < obj + ALPHA * t * np.dot(dg.T, search_dir):
                    break
            ...
            if np.linalg.norm(search_dir) < 1e-7:
                break
            
            if terminate:
                break
    
            x = x.cpu().numpy() + t * search_dir.reshape(-1,)
        ...
        if self.use_ineq and plotlast:
            for f in range(self.F):
                ...
                plt.subplot(131)
                plt.plot(all_lmks[f][:,0], all_lmks[f][:,1], 'o')
...
if __name__ == '__main__':
    ...
    for fov in [12]:
        ...
        pfs = PerspectiveFitter(mm, cam, use_maha=False, which_pts='lmks', F=F, maha2_threshold=300)
        pfd = PerspectiveFitter(mm, cam, use_maha=False, which_pts=which_pts, F=F, maha2_threshold=300)
        # fov = 17
        for subj_id in range(1):# range(100):
