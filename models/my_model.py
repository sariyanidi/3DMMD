...
class MySegregatedModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.backbone_nonrigid = resnets.resnet50()
        self.backbone_rigid = resnets.resnet18()
        self.backbone_illum = resnets.resnet18()
        
        # self.backbone = resnets.resnet50()
        self.backbone_nonrigid.load_state_dict(filter_state_dict(torch.load('./models/checkpoints/resnet50-0676ba61.pth', map_location='cpu')))
        self.backbone_rigid.load_state_dict(filter_state_dict(torch.load('./models/checkpoints/resnet18-f37072fd.pth', map_location='cpu')))
        self.backbone_illum.load_state_dict(filter_state_dict(torch.load('./models/checkpoints/resnet18-f37072fd.pth', map_location='cpu')))
    ...
    def forward(self, input_im, tforms=None, render=False):
        x_nonrigid = self.backbone_nonrigid(input_im)
        x_rigid = self.backbone_rigid(input_im)
        x_illum = self.backbone_illum(input_im)

        output = []
        for layer in self.MM_layers:
            output.append(layer(x_nonrigid))

        for layer in self.illum_layers:
            output.append(layer(x_illum))

        for layer in self.rigid_layers:
            output.append(layer(x_rigid))
    ...
    def freeze_for_rigid_training(self):
        ...
        for param in self.backbone_nonrigid.parameters():
            param.requires_grad = False
            
        for param in self.backbone_illum.parameters():
            param.requires_grad = False
    ...
    def freeze_for_nonrigid_training(self):
        ...
        for param in self.backbone_rigid.parameters():
            param.requires_grad = False
